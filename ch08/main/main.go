...
import (
	...
	"babyagent/ch08"
	ctxengine "babyagent/ch08/context"
	"babyagent/ch08/memory"
	"babyagent/ch08/storage"
	"babyagent/ch08/tool"
	"babyagent/ch08/tui"
	"babyagent/shared"
)
...
func main() {
	...
	mcpClients := make([]*ch08.McpClient, 0)
	for k, v := range mcpServerMap {
		mcpClient := ch08.NewMcpToolProvider(k, v)
	...
	// 配置需要确认的工具
	confirmConfig := ch08.ToolConfirmConfig{
	...
	agent := ch08.NewAgent(
		...
		ch08.CodingAgentSystemPrompt,
		confirmConfig,
		[]tool.Tool{tool.CreateBashTool(shared.GetWorkspaceDir()), tool.NewLoadStorageTool(memoryStorage)},
		mcpClients,
		contextEngine,
	)
