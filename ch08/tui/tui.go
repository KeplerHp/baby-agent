...
func NewModel(agent *ch08.Agent, modelName string) *TuiViewModel {
	vp := viewport.New()
	vp.SoftWrap = true
...
func waitStreamEvent(ch <-chan ch08.MessageVO) tea.Cmd {
	return func() tea.Msg {
		msg, ok := <-ch
		if !ok {
	...
	case "esc":
		if m.state == stateAwaitingConfirmation {
			...
			if m.active != nil && m.active.confirmCh != nil {
				m.active.confirmCh <- ch08.ConfirmReject
			}
...
func (m *TuiViewModel) handleConfirmSelection() (tea.Model, tea.Cmd) {
	...
	var action ch08.ConfirmationAction
	switch m.selectedConfirmIdx {
	case 0:
		action = ch08.ConfirmAllow
	case 1:
		action = ch08.ConfirmReject
	case 2:
		action = ch08.ConfirmAlwaysAllow
	}
...
func (m *TuiViewModel) handleStreamEvent(event ch08.MessageVO) {
	if m.active == nil || m.state == stateAborting {
		return
	}

	switch event.Type {
	case ch08.MessageTypeReasoning:
...
func (m *TuiViewModel) startNewTurn(query string) (tea.Model, tea.Cmd) {
	...
	streamC := make(chan ch08.MessageVO)
	confirmCh := make(chan ch08.ConfirmationAction)
