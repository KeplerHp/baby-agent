...
*   **Language:** Go 1.24+
*   **Concepts:** LLM, Function Calling, ReAct, RAG, MCP, Observability, Context Engineering
*   **Key Skills:** 上下文管理、流式传输、工具调用、外部系统集成、推理模型适配
...
├── ch04/           # ✅ 第四章：MCP 生态接入
├── ch05/           # ✅ 第五章：上下文工程
├── ch06/           # 🚧 第六章：沙盒与安全防御（规划中）
├── ch07/           # 🚧 第七章：Web 服务化与 SSE 流式传输（规划中）
├── ch08/           # 🚧 第八章：服务端状态管理（规划中）
├── ch09/           # 🚧 第九章：Agent 评测与自动化测试（规划中）
├── ch10/           # 🚧 第十章：生产环境保障（规划中）
