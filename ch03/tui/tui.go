...
func NewModel(agent *ch03.Agent, modelName string) *TuiViewModel {
	vp := viewport.New()
	vp.SoftWrap = true
...
func (m *TuiViewModel) handleStreamEvent(event ch03.MessageVO) {
...
func (m *TuiViewModel) startNewTurn(query string) (tea.Model, tea.Cmd) {
	...
	streamC := make(chan ch03.MessageVO)
